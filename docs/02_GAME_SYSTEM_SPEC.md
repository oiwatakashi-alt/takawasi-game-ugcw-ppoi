...
- Battle selected-brigade operation now has a `指揮コンパス`, `担当戦線診断`, and `戦術提案`, and selected-frontline operation has `戦線提案` plus `守備旅団診断`: the UI consolidates current map-click mode, next action instruction, issue/queue state, anchor, fallback, frontline segment, facility assignment, focus target, selected-unit assigned-line pressure/readiness context, selected-frontline defender stress/readiness context, and selected-unit-relevant alert recommendations. It reuses existing StandingOrder/order functions for `基準`, `後退`, `戦線`, `施設`, and `集中` map assignments, reuses the existing alert recommendation command path for selected-unit suggestions, and surfaces frontline-level pressure response, fighting rotation, defender selection, defender rest/resupply, defender fallback-guard, and staff advisory actions in the same panel. The defender diagnosis derives `推奨 維持/休息補給/後退守備` from control-radius, morale, ammo, condition, and strength, then routes `推奨実行` through the same immediate/queued command path. Enemy command-network cards now also show the target frontline pressure/readiness context and expose a direct `対応戦線` action that reuses the frontline pressure-response command path. Staff advisory application and defender diagnosis actions route through existing immediate/queued command paths, so they can be issued immediately or queued while still avoiding new BattleState/save fields.
