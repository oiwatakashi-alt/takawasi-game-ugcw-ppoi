...
- enemies enter from the eastern/right side of the tactical map and move toward an assault-plan destination, nearby player formation, bridge/choke lane, or ridge-crossing maneuver destination
...
- enemy ridge maneuver exists for terrain with explicit `ridgeLine` geometry. `resolveTick` detects enemy movement that crosses the line, applies ridge-height movement penalties, nudges the destination toward a probing or reverse-slope approach, reduces assault cohesion, and exposes terrain tactics such as `稜線越え接近` or `稜線裏滞留` on Battle Command enemy labels.
- enemy terrain maneuver alerts exist: Battle Command raises `敵地形機動` cards for spotted enemies with terrain tactics, carries the enemy ID through alert click/recommendation paths, opens the enemy intent panel to the exact group, and can focus/retask response brigades through existing StandingOrder command logic.
...
- Battle selected-brigade operation now has explicit `選択モード`, a `指揮コンパス`, `担当戦線診断`, and `戦術提案`, and selected-frontline operation has `戦線提案` plus `守備旅団診断`: the UI consolidates current map-click mode, next action instruction, issue/queue state, anchor, fallback, frontline segment, facility assignment, focus target, selected-unit assigned-line pressure/readiness context, selected-frontline defender stress/readiness context, inspected enemy/facility/frontline details, selected-unit-relevant alert recommendations, and inspected-target response forecasts. It reuses existing StandingOrder/order functions for `基準`, `後退`, `戦線`, `施設`, and `集中` map assignments, reuses the existing alert recommendation command path for selected-unit suggestions, and surfaces frontline-level pressure response, fighting rotation, defender selection, defender rest/resupply, defender fallback-guard, staff advisory actions, inspected-enemy response/volley/focus actions with responsible-line and ammo forecasts, inspected-facility defense/repair actions with candidate-unit and repair forecasts, and selected-frontline response actions with pressure/defender/reserve forecasts in the same panel. The selected map inspector now also has order-transfer actions and advisory comparison cards: `敵前縁へ布陣` derives an anchor/fallback pair from the clicked enemy and focuses it when spotted, `選択旅団を戦線へ` copies the selected frontline assignment plus anchor/fallback, and `選択旅団を担当` / `施設近接へ基準` bind the inspected facility to the selected brigade; `比較 敵前縁`, `比較 戦線転属`, and `比較 施設近接` score those choices by move distance, fallback depth, line pressure, defenders/reserves, facility value, durability, tactical pressure, and overrun risk. Those comparison cards also display `推奨/注意/危険` reason text and the expected `戦果役割` label before execution, so the player can compare both tactical value and After Action carryover. The defender diagnosis derives `推奨 維持/休息補給/後退守備` from control-radius, morale, ammo, condition, and strength, then routes `推奨実行` through the same immediate/queued command path. Enemy command-network cards now also show the target frontline pressure/readiness context, command inheritance such as `継承 第2波指揮核 -> 2群`, isolation count, and collapse-risk readout, and expose a direct `対応戦線` action that reuses the frontline pressure-response command path. Staff advisory application, defender diagnosis actions, map-inspection response actions, and map-inspection order transfers route through existing immediate/queued command paths, so they can be issued immediately or queued while still avoiding save fields. Map-inspection response and order-transfer roles are battle-only but carry into BattleResult as `検査敵群対応`, `検査戦線斉射`, `検査集中射撃`, `検査施設防衛`, `検査施設修理`, `検査戦線対応`, `検査敵前縁布陣`, `検査戦線転属`, `検査施設担当`, or `検査施設基準`, giving After Action, officer events, division summaries, campaign unit history, and facility-duty lesson previews a record of the player's inspected-map intervention.
- The selected-frontline defender diagnosis also has line-scoped bulk actions. `疲弊を休息補給` gathers defenders whose recommendation is rest and applies/queues `setUnitOrder(..., "rest")`; `危険を後退守備` gathers defenders whose recommendation is fallback and applies/queues `applyStandingOrderPreset(..., "fallback_guard")`. These actions use `issueOrQueueBattleCommand`, select the first affected brigade for map context, and add no BattleState/save fields.
- Battle command queue exists as local Battle Command UI state: `予約指揮` can pause the battle, queue selected-brigade posture/target/ammo/facing/immediate-order/fire-control changes, tactical-map commands for anchor/fallback/frontline/facility/focus-target assignment, enemy-intent panel commands for focus target/responsible-frontline response/line volley, enemy command-network group actions for command-node fire/collapse pursuit/reserve commitment, alert-card recommendations, frontline-pressure responses, and pressure-card/default fighting rotations, delete or discard queued items, and then apply them through existing `orders.ts` functions with `一括発令`. Those order functions now mark battle-only pending orders with a short command-transmission delay derived from local contact pressure, reserve readiness, morale, movement, command-overload summary, command/organization doctrine reduction, and the next-battle `commandPost` profile derived from the assigned `参謀長` fatigue/status; Deployment shows this same profile before battle as `司令部伝達` and warns with `司令部伝達警告` plus `将校調整へ` when it will degrade transmission or queue capacity. Deployment also lets the player save a command issue plan before battle: `標準発令` keeps the existing queue behavior, `分割発令` treats a larger queue as smaller batches for congestion capacity, and `逐次発令` makes multi-command batches a deliberate policy violation with an extra delay penalty. A `伝令運用` advisor now reads expected batch size, command/organization doctrine capacity, and command-post fatigue, then recommends standard/split/direct handling with reasons and a one-click adoption button, so the player is not choosing the policy blind. Battle Command displays the active plan in `予約指揮`, mirrors it with a live compliance line (`方針待機`, `方針適合`, `分割適用`, `混線注意`, `逐次違反`) before `一括発令`, provides a policy execution button for split/direct plans that issues only the first allowed batch while leaving the rest of the queue intact, and named `戦線計画セット` saves/restores that plan with the rest of the deployment package. While a command is still being carried, `resolveTick` applies small movement/fire penalties and Battle Command shows `伝令 N秒 / ...` chips until arrival. Battle Command also shows pre-issue `伝令予測`, queued-command forecast rows, `伝令判断` reasons such as `即応53で+1秒`, `指揮幕僚-1秒`, or `司令部疲労+2秒`, and `伝令影響` penalty text, so the player can see why command discipline matters. `一括発令` also has command-post congestion: if queued command count exceeds doctrine-, commandPost-, and command-issue-plan-modified handling capacity, `orders.ts` adds `一括混線 +N秒` to same-moment pending orders and logs the reason; policy partial execution calculates that congestion from the issued subset rather than from commands left waiting in the queue. Issued commands are additionally stored as battle-only `commandTransmissionEvents`; `BattleResult.commandTransmissionOutcomes` carries delay/congestion/arrival assessments into After Action, campaign last message, unit history, brigade-commander fatigue/XP/history, chief-of-staff warning history, staff recommendation `前戦評価`, and next-battle tactical lessons. This command-event state is intentionally not persisted as a save/migration field, while the pre-battle command issue plan is persisted in deployment state.
...
- direct drag deployment/movement remains deferred, but segment-scoped frontage sketching now supports up to 8 control points, desktop drag capture, compacted point-list storage, and smooth curve rendering for 3+ point lines; commands, preset geometry, handle controls, and Battle Command/Deployment `戦線スケッチ` modes drive automatic movement
...
- Staff advisory outcomes are now part of the battle-to-campaign growth loop. `createBattleResult` evaluates accepted advisories as `戦線維持に寄与 / 撤退支援 / 対応及ばず`, gives involved units a small XP bonus, and After Action shows both the overall advisory result, per-unit advisory entries, and a `次戦教訓` preview for the involved units. `applyBattleResult` adds the first advisory summary to campaign history/message and records involved advisory outcomes into each unit's `battleHistory`. Command-transmission outcomes now follow the same loop at lower weight: After Action shows `伝令評価`, `applyBattleResult` writes `伝令評価 ... / 混線+N秒 / 到達 or 未着` into unit history, adds `伝令混線` officer history/fatigue to the brigade commander, adds `参謀長 警告、伝令混線` to the chief of staff when command-post congestion affects the army, and `tacticalLessons.ts` converts unit histories into small readiness/control/fallback bonuses.
...
- `StandingOrderTemplate` is persisted in the campaign save path; current local saves are v8 after adding Deployment withdrawal rear-guard planning. Battle Command can save a selected brigade's autonomous order, and the selected-frontline panel can save all current defenders on that line as frontline-plan templates after direct line edits or up-to-8-point `戦線スケッチ`. Confirmed battle sketches can also copy their compacted `frontlineSketchPoints` into each saved per-brigade template, so Army Camp and Deployment can show `戦線形状 N点`. Deployment can promote those saved sketch points into `FrontlineGeometryAdjustment.sketchLines` when a plan set is saved, and plan-set apply rehydrates Deployment/Battle segments through `applyFrontlineGeometryAdjustment`. Deployment displays saved-order coverage, compares the selected brigade's current pre-battle draft against its saved standard order, shows changed areas, and can restore the draft from the saved template. Army Camp can clear a selected brigade's saved standard order, and battle creation applies saved brigade templates.
- `StandingOrderPlanSet` is persisted in the campaign save path from v7 and now uses save v8 for withdrawal rear-guard planning. Deployment can save the current selected battle plan as a named set containing frontline geometry, reserve doctrine, designated reserves, designated withdrawal rear guards, and all selected brigades' current StandingOrders. Plan sets now clone `frontlineGeometry.sketchLines`, show `形状N線` in their summary, and preview saved sketch frontage such as `保存戦線形状 森林左翼線 8点`. Deployment can create these segment-scoped sketch lines directly from the selected frontline handle editor with `地図で描画`, `1点戻す`, `描画確定`, `既存線を編集`, `現線を形状化`, `北寄せ形状`, `南寄せ形状`, and `形状解除`; map-click/drag drawing accepts up to 8 points, stores the same compact point-list format, and renders 3+ point frontage as smooth SVG paths before saving. The `戦線計画セット` panel can preview matched/missing brigades, changed StandingOrders, saved battlefield, frontline geometry, reserve doctrine, designated reserves, designated rear guards, and saved segment-scoped sketch shapes; detailed preview rows show per-brigade changed fields plus saved posture/target/ammo summary. The player can rename, overwrite, apply, or delete each set. Applying a plan set restores matching deployed brigades into the pre-battle draft and updates the deployment plan, without overwriting per-brigade standard templates until the player explicitly saves those.
...
- Frontline terrain assessment now connects terrain/structures to line-placement decisions. `src/game/battle/frontlineTerrainAssessment.ts` scores each frontline segment for fire advantage, cover value, mobility risk, support value, and ridge risk, then returns Japanese tags, reasons, and a suggested frontline doctrine. Explicit `ridgeLine` terrain can produce `稜線射界`, `稜線支配`, or `稜線越え` assessments; `稜線越え` lowers geometry confidence and pushes weak-line mitigation toward cautious fallback rules. The same module aggregates those assessments by frontline geometry preset, so Deployment can compare `戦区標準`, `前進主線`, `縦深防御`, `広域警戒線`, and other shapes by average score, weakest line score, ridge risk, tone, and recommended doctrine before applying a geometry. It also builds a weak-line mitigation advisory for Deployment, identifying the lowest-scoring line and recommending geometry changes, reserve support, deeper fallback, facility support, or fire posture where relevant. Deployment can apply that advisory into saved StandingOrderTemplates: the focus brigade is moved to the weak line with a fitting posture/ammo/fallback/facility assignment, and a support brigade can be designated as reserve. Battle creation marks those brigades as deployment-mitigation units, so After Action and unit history can later record `弱線是正` or `弱線支援予備` rather than losing the pre-battle duty after battle starts. Deployment shows segment assessments before battle as per-line cards and preview badges, including URL-gated tactical profile terrain such as `逆斜面射線検証`; Battle Command shows the selected line's live `地形評価` and `地形判断` next to the selected-frontline doctrine board.
...
- Terrain line-of-sight now has tactical modifiers, not only blocked/clear states. `src/game/battle/terrainEffects.ts` classifies forest/village as blocking terrain and hill/marsh as partial attenuation, then adds `高地射界`, `遮蔽端射撃`, low-posture deep cover, enemy cover-edge, enemy ridge, reverse-slope, and line-shaped ridge-crossing modifiers with range/fire multipliers. Hill terrain can carry explicit `ridgeLine` geometry; shots crossing that crest show `稜線越え ...` and receive additional ridge-height-based blockage, fire, and range penalties. `visibility.ts` still reduces or blocks long-range spotting through those zones, while `resolveTick` excludes blocked targets and applies LOS range/fire multipliers to brigade target checks and damage. Battle Command labels terrain as `射線遮蔽`, `射線減衰`, `遮蔽端`, or `高地射界 / 稜線`, renders ridge overlays as dashed `稜線` lines, unit cards show current `射線` state, and target-audit rows show per-candidate `有効射程`. The URL-gated tactical terrain profiles `?takawasiTerrainProfile=high-ground` and `?takawasiTerrainProfile=reverse-slope` create reproducible `高地射線検証` and `逆斜面射線検証` battle profiles with QA-only terrain/enemy entry points, so high-ground, reverse-slope, and crest-crossing LOS can be browser-verified without changing campaign save state.
- Enemy movement also reads the same ridge-line geometry. Enemy groups crossing a ridge can slow, lose cohesion, and surface terrain-action labels such as `稜線裏滞留`, so elevation affects assault behavior as well as player fire and visibility.
- Battle alerts now surface those terrain actions as command decisions. A `稜線裏滞留` enemy can trigger `戦線で封鎖`, selecting the exact enemy, showing the `地形行動` detail row, and applying elastic-defense/focus-target response without adding persistent save fields. Terrain-alert responders are now marked with battle-only result roles: ridge blocking becomes `地形敵封鎖`, other terrain focus responses become `地形敵阻止`. After Action displays those roles and commendations, campaign history stores them, and Army Camp tactical lessons summarize them as `地形敵対応N件`, preferring `弾性拒止` for ridge-blocking and `殺傷地帯` for terrain-focus learning.
...
- Objective-event response results now exist in the after-action growth loop. `BattleObjectiveEventState` also carries chain degradation (`命令混線`, `指揮崩壊拡大`, `補給路寸断`, `弾薬誘爆`, `死角拡大`, `霧中突破`) when the same strained/critical objective event remains unrecovered, and those chain stages add extra line/ammo/spotting penalties through tactical effects. `BattleResult.objectiveEventResponseOutcomes` evaluates objective-response units against final objective control/event severity/chain state as `再確保`, `遅滞`, or `未回復`; After Action displays the event and chain labels, and `tacticalLessons.ts` turns those records into next-battle readiness/control/fallback lessons. Deployment treats objective-event-only lessons as actionable when they infer a preferred doctrine, so the player can apply or save lessons from failed/delayed objective events before the next mandatory battle.
...
- Enemy officer command now exists for undead waves. `src/game/battle/waves.ts` spawns undead officers on command waves, assigns command roles/tiers/intents/groups/labels such as `第2波指揮核`, `波指揮核`, `突撃先導`, `支援節`, and `前衛群`. Spawned enemies are also linked through battle-only command inheritance, so `前衛群` can inherit from `突撃先導`, `支援節`, or `波指揮核`. `resolveTick` calculates command influence, command-intent effects, command-tier pressure/recovery modifiers, and command-collapse state, and Battle Command shows `敵指揮網`, `指揮下`, tier summaries, inheritance/isolation/collapse-risk readouts, `指令 戦線圧迫/側面迂回/陣地突破/銃列支援/再集結`, and `敵指揮崩壊` signals. The enemy command-network summary also provides `指揮核射撃`, `崩壊追撃`, and `予備投入` commands that can be queued through the same battle-only command queue, while the card-level `推奨実行` picks one of those actions from command tier, pressure, pursuit opportunity, lead threat, breakthrough/facility context, and command-inheritance risk. High inheritance risk can produce `推奨 継承遮断`; executing that recommendation now uses a distinct battle-only inheritance-cut role while explicit `指揮核射撃` remains normal command-node fire. Cards also show a pre-execution forecast such as `予測 指揮低下 / 2旅団 / 弾薬高`, `予測 継承遮断 / 2旅団 / 弾薬高`, or `予測 突破封鎖 / 予備1 / 即応-32`, so the player can judge expected effect and cost before committing. Once an action is active, the same card can show `効果中 指揮核射撃`, `効果中 継承遮断`, `効果中 崩壊追撃`, or `効果中 予備接続` from the participating brigades' battle-runtime roles and disables the matching action to prevent accidental double commitment. The response-effect row then measures the action: command-node fire reports `効果 指揮崩壊...`, inheritance cut reports `効果 継承遮断候補...`, collapse pursuit reports `効果 再集結抑止...`, and reserve commitment reports `効果 戦線補強 / 守備... / 1旅団圧...`.
- Enemy command-network actions now carry into battle results. `BattleUnit.enemyCommandActionRole` maps command-node fire, inheritance cut, collapse pursuit, and command-network reserve commitment into After Action roles `敵指揮核制圧`, `敵継承遮断`, `敵崩壊追撃`, and `指揮網予備投入`, with XP/officer XP and unit-history commendations. Battle Command's enemy command-network cards now expose `追撃候補` with target/reason/cohesion, so `崩壊追撃` is a readable battlefield opportunity for command-disrupted, wavering/routing/regrouping, breakthrough/flanking/overextended, low-cohesion, or small-group cleanup targets. `BattleResult.enemyCommandEffectOutcomes` separately records the measured response effect, such as `封鎖安定`, `継承遮断`, `指揮低下`, or `再集結抑止`, with metric labels, lesson tags, and assessment reasons.
- Enemy command-network action and effect history now feeds next-battle tactical lessons. `敵指揮核制圧` and `敵継承遮断` teach command suppression/inheritance-cut lessons that currently map to `遅滞節約`, `敵崩壊追撃` maps toward `殺傷地帯`, and `指揮網予備投入` maps toward `弾性拒止`; measured `指揮網効果` entries add smaller reserve-readiness/control-radius growth and show as `次戦教訓 指揮網効果...` in After Action. Deployment can apply or save those lessons through the same `教訓方針` controls as staff-advisory lessons.
...
- Deployment drag sketch drawing now exists for desktop. While `地図で描画` is active, the deployment preview accepts a dragged line, samples it, and compacts it into the existing `FrontlineGeometryAdjustment.sketchLines` point-list format with an 8-point cap. Three or more points render as a smooth SVG curve in Deployment and Battle Command, while two-point lines remain straight. This keeps click point-editing, `既存線を編集`, plan-set save/apply, and Battle carryover compatible while making pre-battle frontline intent faster to express.
