...
- Battle selected-brigade operation now has a `指揮コンパス`: the UI consolidates current map-click mode, next action instruction, issue/queue state, anchor, fallback, frontline segment, facility assignment, and focus target, while reusing the existing StandingOrder/order functions for `基準`, `後退`, `戦線`, `施設`, and `集中` map assignments.
