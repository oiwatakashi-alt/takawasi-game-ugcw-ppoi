...
- Battle alerts now surface those terrain actions as command decisions. A `稜線裏滞留` enemy can trigger `戦線で封鎖`, selecting the exact enemy, showing the `地形行動` detail row, and applying elastic-defense/focus-target response without adding persistent save fields.
