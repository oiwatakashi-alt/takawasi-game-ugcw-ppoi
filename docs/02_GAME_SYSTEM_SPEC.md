...
- Battle selected-brigade operation now has a `指揮コンパス` plus `戦術提案`: the UI consolidates current map-click mode, next action instruction, issue/queue state, anchor, fallback, frontline segment, facility assignment, focus target, and selected-unit-relevant alert recommendations. It reuses existing StandingOrder/order functions for `基準`, `後退`, `戦線`, `施設`, and `集中` map assignments, and reuses the existing alert recommendation command path so suggested responses can be issued immediately or queued without adding new BattleState/save fields.
