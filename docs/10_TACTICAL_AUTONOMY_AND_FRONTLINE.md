...
- `npm run build` and `git diff --check` passed. Browser QA for the selection-mode pass verified Theater -> Army Camp -> Deployment -> Battle Command, `command-select`, structure inspection, enemy inspection without focus-target assignment, player-unit reselection, console errors/warnings 0, broken images 0, horizontal overflow false, and no visible `NaN`. The follow-up response-action QA verified structure inspection -> `施設即応` log and enemy inspection -> `敵群対応` log from the same inspector. Forecast QA verified structure forecasts, enemy forecasts after 3x wave spawn, ready/warning/danger tones, and collapse-state button disabling while forecast cards stayed visible. Carryover QA verified enemy inspection response -> withdraw -> After Action with `任務 検査敵群対応`, commendations `検査敵群へ即応` and `確認敵群を指名`, and responding-brigade officer events using the same role. Earlier command-compass QA remains recorded in `outputs/takawasi-selected-command-compass-qa-report.json`, `outputs/takawasi-selected-tactical-suggestions-qa-report.json`, `outputs/takawasi-selected-frontline-suggestions-qa-report.json`, `outputs/takawasi-enemy-command-linked-frontline-qa-report.json`, `outputs/takawasi-selected-unit-frontline-diagnosis-qa-report.json`, `outputs/takawasi-selected-frontline-defender-diagnosis-qa-report.json`, and `outputs/takawasi-selected-frontline-defender-bulk-command-qa-report.json`; selection-mode QA is recorded in `outputs/takawasi-map-selection-mode-qa-report.json`, response-action QA in `outputs/takawasi-map-selection-response-qa-report.json`, forecast QA in `outputs/takawasi-map-selection-forecast-qa-report.json`, and carryover QA in `outputs/takawasi-map-selection-after-action-carryover-qa-report.json`. Mobile/cellphone QA is outside the current target.
