...
- Map-inspector-origin responses now carry into After Action. `BattleUnit.mapInspectionResponseRole` is battle-only and records `検査敵群対応`, `検査戦線斉射`, `検査集中射撃`, `検査施設防衛`, `検査施設修理`, or `検査戦線対応`; `createBattleResult` turns that into `battleRoleByUnit`, role XP, commendations, officer events, and campaign unit history. Facility inspection roles also feed the existing facility-duty tactical lesson preview.
