...
- The selected-frontline panel also shows `守備旅団診断`, sorted by each defender's stress score. It lists morale, ammo, strength ratio, posture, target priority, anchor distance, and warnings such as low morale, low ammo, or outside control radius, then selects and scrolls to the exact brigade for detailed command.
