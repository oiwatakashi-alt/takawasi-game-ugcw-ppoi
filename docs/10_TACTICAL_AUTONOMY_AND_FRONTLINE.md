...
- Battle Command now also has explicit `選択モード`. It is separate from command-issuing modes, auto-shows frontline and facility layers, highlights clickable tactical entities, and uses `map-selection-inspector` to summarize the selected brigade, inspected enemy, selected frontline, and inspected/assigned facility. In this mode, clicking a structure inspects it without assigning facility duty, and clicking an enemy inspects it without setting a focus target.
- The map selection inspector now also provides direct response actions for the inspected target. Clicked enemies expose `敵群対応`, `戦線斉射`, and `選択旅団集中`; inspected/assigned facilities expose `施設即応` and `修理優先`; the selected frontline exposes `戦線対応` when pressure is actionable. These buttons reuse the existing selected-enemy response, facility-defense response, frontline-pressure response, and immediate/queued command paths rather than adding new BattleState or save fields.
- The same inspector now shows response forecast cards before the buttons. Enemy forecasts show responsible frontline, response brigade count, posture/target priority, volley issuer/ammo, and selected-brigade focus feasibility; facility forecasts show response unit count, facility state, tactical pressure, repair rate, candidate units, engineer availability, and repair need; frontline forecasts show recommendation, defender/reserve counts, pressure level, and lead threat. Forecast cards use `ready/warning/danger` tones and read the same calculations that the buttons execute.
- The inspector can now also turn inspected map objects into selected-brigade StandingOrder changes. `敵前縁へ布陣` sets the selected brigade's anchor to a defensive point in front of the clicked enemy, sets a deeper fallback point, and focuses the enemy if spotted; `選択旅団を戦線へ` assigns the selected frontline plus that segment's anchor/fallback; `選択旅団を担当` and `施設近接へ基準` bind the inspected/assigned facility to the selected brigade, with the latter also moving the anchor near the facility. These are battle-time order transfers only and reuse command transmission/queue behavior.
- The inspector now compares selected-brigade order-transfer candidates before execution. `比較 敵前縁` scores enemy-front movement from move distance, fallback depth, spotted state, and enemy phase/command risk; `比較 戦線転属` scores selected-frontline transfer from move distance, current line pressure, defenders, and reserves; `比較 施設近接` scores facility anchoring from move distance, facility value, durability, tactical pressure, and overrun state. The cards show `推奨/注意/危険` reason text plus the expected `戦果役割` such as `検査敵前縁布陣`, `検査戦線転属`, or `検査施設基準`, but remain advisory-only and keep execution on the existing buttons.
- Map-inspector-origin responses and order transfers now carry into After Action. `BattleUnit.mapInspectionResponseRole` is battle-only and records `検査敵群対応`, `検査戦線斉射`, `検査集中射撃`, `検査施設防衛`, `検査施設修理`, `検査戦線対応`, `検査敵前縁布陣`, `検査戦線転属`, `検査施設担当`, or `検査施設基準`; `createBattleResult` turns that into `battleRoleByUnit`, role XP, commendations, officer events, division commander summaries, and campaign unit history. Facility inspection roles and facility order-transfer roles also feed the existing facility-duty tactical lesson preview.
...
- The selected-brigade panel now also shows `担当戦線診断`. It reads the existing frontline pressure report for the selected brigade's assigned segment, shows the brigade's role, enemy pressure, defender/reserve counts, distance to the line anchor, main threat, and readiness warnings, then links to frontline inspection or the existing pressure-response command path.
...
- The selected-frontline panel now shows `戦線提案`, comparing pressure response, fighting rotation, and staff advisory action for the active frontline. Pressure response and rotation already used queued battle commands; staff advisory application now also routes through `issueOrQueueBattleCommand`, so a paused player can queue staff doctrine responses while preserving the same `staffAdvisoryResponses` after-action carryover.
- The selected-frontline panel also shows `守備旅団診断`, sorted by each defender's stress score. It lists morale, ammo, strength ratio, posture, target priority, anchor distance, and warnings such as low morale, low ammo, or outside control radius, then derives `推奨 維持/休息補給/後退守備` with a short reason. The card selects and scrolls to the exact brigade, and can directly issue/queue `推奨実行`, `休息補給`, or `後退守備` through `issueOrQueueCommand`, so line-level diagnosis now leads to existing rest/fallback-guard command transmission instead of being only a readout.
- The same defender diagnosis now exposes selected-line bulk command buttons. `疲弊を休息補給` issues or queues rest only for defenders whose diagnosis recommends rest, and `危険を後退守備` issues or queues fallback guard only for defenders whose diagnosis recommends fallback. Both buttons route through `issueOrQueueBattleCommand` and existing `setUnitOrder` / `applyStandingOrderPreset` functions, so they add no new BattleState/save fields while making large-line command practical.
- Enemy command-network cards now show linked frontline pressure, defenders, reserves, and the frontline recommendation. Their new `対応戦線` action calls the same frontline pressure-response path, allowing enemy command groups to be answered by the corresponding frontline and queued under `予約指揮`.
...
- `npm run build` and `git diff --check` passed. Browser QA for the selection-mode pass verified Theater -> Army Camp -> Deployment -> Battle Command, `command-select`, structure inspection, enemy inspection without focus-target assignment, player-unit reselection, console errors/warnings 0, broken images 0, horizontal overflow false, and no visible `NaN`. The follow-up response-action QA verified structure inspection -> `施設即応` log and enemy inspection -> `敵群対応` log from the same inspector. Forecast QA verified structure forecasts, enemy forecasts after 3x wave spawn, ready/warning/danger tones, and collapse-state button disabling while forecast cards stayed visible. Carryover QA verified enemy inspection response -> withdraw -> After Action with `任務 検査敵群対応`, commendations `検査敵群へ即応` and `確認敵群を指名`, and responding-brigade officer events using the same role. Order-transfer QA verified `施設近接へ基準`, `選択旅団を戦線へ`, enemy spawn/inspection, `敵前縁布陣予測`, and `敵前縁へ布陣`, with console errors/warnings 0, broken images 0, horizontal overflow false, and no visible `NaN`. Order-transfer carryover QA verified enemy inspection -> `敵前縁へ布陣` -> withdraw -> After Action with `任務 検査敵前縁布陣`, commendations `確認敵群へ前縁布陣` and `敵群を基準目標化`, brigade officer event `検査敵前縁布陣、指揮経験+5`, and division commander summary including the same role. Order-transfer comparison QA verified three advisory cards, `比較 敵前縁`, `比較 戦線転属`, and `比較 施設近接`, plus existing `敵前縁へ布陣` execution. Comparison-role QA verified recommendation text and expected `戦果役割` labels for `検査敵前縁布陣`, `検査戦線転属`, and `検査施設基準`, then confirmed `敵前縁へ布陣` still updated anchor/fallback and pending transmission. Earlier command-compass QA remains recorded in `outputs/takawasi-selected-command-compass-qa-report.json`, `outputs/takawasi-selected-tactical-suggestions-qa-report.json`, `outputs/takawasi-selected-frontline-suggestions-qa-report.json`, `outputs/takawasi-enemy-command-linked-frontline-qa-report.json`, `outputs/takawasi-selected-unit-frontline-diagnosis-qa-report.json`, `outputs/takawasi-selected-frontline-defender-diagnosis-qa-report.json`, and `outputs/takawasi-selected-frontline-defender-bulk-command-qa-report.json`; selection-mode QA is recorded in `outputs/takawasi-map-selection-mode-qa-report.json`, response-action QA in `outputs/takawasi-map-selection-response-qa-report.json`, forecast QA in `outputs/takawasi-map-selection-forecast-qa-report.json`, carryover QA in `outputs/takawasi-map-selection-after-action-carryover-qa-report.json`, order-transfer QA in `outputs/takawasi-map-inspection-order-transfer-qa-report.json`, order-transfer carryover QA in `outputs/takawasi-map-inspection-order-transfer-carryover-qa-report.json`, order-transfer comparison QA in `outputs/takawasi-map-inspection-order-comparison-qa-report.json`, and comparison-role QA in `outputs/takawasi-map-inspection-order-comparison-role-qa-report.json`. Mobile/cellphone QA is outside the current target.
...
## Implemented Enemy Command Inheritance Carryover Slice - 2026-07-05

`継承遮断` now survives as a tactical result and growth lesson, not only as a recommendation/forecast row.

Implemented:

- Battle Command recommendation execution now marks participating brigades with battle-only `command_inheritance_cut` when the card recommendation is `推奨 継承遮断`.
- Active cards show `効果中 継承遮断` and a measured effect row such as `効果 継承遮断候補...`, while the explicit `指揮核射撃` button remains the normal command-node fire action.
- `BattleResult.enemyCommandEffectOutcomes` now records `敵継承遮断` with result labels such as `継承遮断`, `指揮低下`, or `効果限定`, plus metric labels for dependent command groups, unstable groups, influence, and cohesion.
- After Action displays the inheritance-cut result in `敵指揮網対応評価`, per-brigade `敵指揮網 ... 教訓 ...` rows, and `次戦教訓 指揮網効果...` previews.
