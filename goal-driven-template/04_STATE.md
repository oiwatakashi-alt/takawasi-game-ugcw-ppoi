...
- 現マイルストーン: 個人VPS preflight / SHA別release / live QA / rollback
- 直前に完了したこと: ユーザーが`条件付き続行`を判定。コンテンツ量は一旦十分、再戦価値は整形後に再評価する条件で、M5検収パックを確定しhumanopsをdoneへ移す。Scenario Pack schema/validator、作成ファクトリ、`border-emergency-001`、作成手順書を追加し`npm run build`を通過。UGCW比較で固定したP0/P1順に従い、Battle active save/reload、終了BattleのAfter Action復元、seed付きeffect log、5tick決定的replay fixture、Battle viewportのsummary/detailと初回ガイドを接続し、Scenario Pack起点のlocal一周をTheater→Camp→Deployment→Battle→After Action→Camp→第3戦略ターンのTheaterまで再走した。Battleで`勝利点保持`を発令し、After Actionの資源消費・勝利点喪失・補給点保持・部隊/将校結果を確認。次ターン直前報告に制御撤退の影響が伝播し、console error 0・broken image 0・水平overflowなしを終端画面で確認
- 次の一手: `docs/deployment/PERSONAL_VPS_STATIC.md`のゲートに従い、未コミット変更を閉じて個人GitHubへpushし、read-only preflight後にSHA別release/current/nginx -t/rendered smoke/live desktop QA/rollback確認を実行する
- ブロッカー: 技術的ブロッカーなし。local一周とBattle viewport受入は緑。VPSの今回SHA公開とlive一周が未達。content量産・画像追加・外部AIモデル委譲は停止
...
- 現在ウェーブ: 006_personal_VPS_preflight_and_release(開始: 2026-07-10。local一周QA完了後の01_PLANフェーズ2項目)
...
- viewport summary/detail・初回説明・命令→動作→結果の因果が条件付き続行の受入を満たすか / フェーズ2 UI整形
...
- 2026-07-10 / Battleのマップ実幅1680pxは水平overflowを発生させなくてもミニマップと右側判断材料を1280px画面の外へ逃がしていた。デスクトップではマップをviewport幅へ収め、部隊カードだけをclampで縮める方が、主戦場と判断材料を同一スクリーンショットへ戻せる
...
- 2026-07-10 / B契約受入経路 / ユーザー条件の「seed/replayまたはevent/effect log」に対し、seed付きeffect log＋決定的tick replayを採用。5tickの同一入力結果digest一致を確認し、完全な命令入力リプレイは追加拡張へ分離 / 次はviewportと初回説明へ移る
- 2026-07-10 / Battle viewport受入 / 1280x720では`topbar→command→初回ガイド→警報→主戦場map→HUD`を固定し、マップのインライン1680px指定を撤去してviewport幅へ収めた。6指標は常設、波計画・視界・隘路はsummary/detail、初回ガイドは命令→動作→結果を説明する / `outputs/ui-reorg/battle-viewport-v6-2026-07-10.json`を原本とし、次はScenario Pack起点local一周QA
- 2026-07-10 / local一周の因果確認 / `勝利点保持`→撤退→After Action→第3戦略ターンの直前報告までを同一操作列で確認した。勝利ではなく制御撤退でも、勝利地点・補給点・追撃被害・責任が後続画面へ伝播する / `outputs/takawasi-local-loop-qa-v6-2026-07-10.json`を原本とし、次は個人VPS preflight
