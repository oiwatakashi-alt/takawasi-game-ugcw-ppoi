...
- 直前に完了したこと: release SHA `d3cb6e97c5ade8e73e489c9814c476cc92a14d66` を92ファイルのhash一致で個人VPSへ配置し、専用vhost、`nginx -t`、reload、rendered index smokeを通過。ただしlive 1280x720でTheater主操作「幕舎で準備する」がtop=810となりファーストビュー外だったため、UI受入は未完了として修正へ戻した
- 次の一手: Theater主操作を主戦場タイトル直後へ移した新SHAを再buildし、local/live 1280 desktopで主操作top、右側補助面、console error 0、broken image 0、overflowなしを再検証する。M5の人間判定は別軸として未完了のまま保持する
...
- 現在ウェーブ: 013_Theater主操作上端修正・個人VPS再公開待ち(開始: 2026-07-10。計画は01_PLANのM3/UI項目)
