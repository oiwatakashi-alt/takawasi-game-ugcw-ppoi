...
- ブロッカー: TCP/22はread-only再確認でもopen、既存ローカルSSH経路はauth_denied。HTTPSはread-only再確認でも404、release/vhost/reloadは未実行。証跡: `outputs/takawasi-vps-deploy-blocked-2026-07-10.md` / `outputs/takawasi-vps-preflight-1ce85dd.json`

## 予算カウンタ

以下の行は `tools/check.sh` が読む。行頭の書式(「- テスト数: 」等)を崩さず、数値は半角で書く。

- このフェーズの消費ターン: 11 / 目安12
